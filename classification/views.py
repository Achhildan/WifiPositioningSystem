...
class GetLokasi(generics.ListAPIView):
    queryset = DataWiPos.objects.all()
    serializer_class = DataWiPosSerializer
    filter_backends = [DjangoFilterBackend]
    filterset_fields = ['username']
    ...
    def list(self, request, *args, **kwargs):
        ...
        # Extract the 'lokasi' field from each serialized object
        lokasi_data = [item['lokasi'] for item in serializer.data]
        # Remove duplicates by converting the list to a set and back to a list
        unique_lokasi_data = list(set(lokasi_data))
        # Create a list of labeled locations
        labeled_locations = {f"ruangan{i+1}": location for i, location in enumerate(unique_lokasi_data)}

        response = {"data": unique_lokasi_data}
        return Response(data=response, status=status.HTTP_200_OK)
...
class PredictModel(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        print(f"Program Predict dijalankan dalam waktu: {elapsed_time:.2f} detik")
        ...
        end_time_predict = time.time()  # Mencatat waktu selesai
        elapsed_time = end_time_predict - start_time_predict  # Menghitung selisih waktu
        print(f"Program Prediksi dijalankan dalam waktu: {elapsed_time:.2f} detik")
